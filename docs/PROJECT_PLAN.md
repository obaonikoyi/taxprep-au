...
| **26: reading an unsupported bank statement** | Transcribe a statement layout this app does not document, and import it only when it adds up to the statement's own opening balance, closing balance and printed totals | Engineering implemented, off unless a key is configured and off unless the person ticks the box per file; changes an on-screen promise; [the argument, the checksum and the hole it leaves](MILESTONE_26_READING_AN_UNSUPPORTED_STATEMENT.md) |
| **27: a pay rise is not a shortfall** | Treat a payslip above the recorded rate as a record to update rather than a question for payroll, with one click to record the new rate from that period | Engineering implemented, on for everyone; [why a panel that cries wolf is worse than silence](MILESTONE_27_A_RISE_IS_NOT_A_SHORTFALL.md) |
| **28: a front door a stranger can use** | Lead with the file picker in plain words, ask about the assisted reader only when a layout is not recognised and only about that file by name, and bring the page to anything the app says | Engineering implemented, on for everyone; fixes a reported dead end where the answer rendered 427px above the viewport; [the two faults and where the consent went](MILESTONE_28_A_FRONT_DOOR_A_STRANGER_CAN_USE.md) |
