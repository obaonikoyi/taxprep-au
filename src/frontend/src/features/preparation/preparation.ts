...
export const situationQuestions = [
  ...
  { key: 'medicare', label: 'Entitled to Medicare for the whole year?', normal: 'yes', help: 'The tax illustration supports full-year entitlement only. Exemptions are checked separately.', gap: 'Medicare eligibility, reductions or exemptions need review.' },
  ...
  { key: 'specialRates', label: 'A working holiday maker or subject to another special tax-rate arrangement?', normal: 'no', help: 'The calculation uses ordinary full-year resident rates only. Choose unsure if a special rate or treaty treatment may apply.', gap: 'Working holiday maker or other special tax-rate treatment needs separate review.' },
  { key: 'deductions', label: 'Any deductions to include, including phone expenses?', normal: 'no', help: 'The first tax illustration supports no deductions. Phone work portions remain separate draft assessments.', gap: 'Deductions need qualified assessment before an overall tax calculation.' },
  { key: 'sapto', label: 'Eligible for the seniors and pensioners tax offset (SAPTO)?', normal: 'no', help: 'Choose unsure if eligibility needs checking. SAPTO and its Medicare thresholds are outside this version.', gap: 'SAPTO eligibility and its Medicare thresholds need separate calculation.' },
  { key: 'levyExemption', label: 'Any full or half Medicare levy exemption days?', normal: 'no', help: 'Includes qualifying medical conditions, foreign-resident periods or periods without Medicare entitlement.', gap: 'Full or partial Medicare levy exemptions need separate calculation.' },
  { key: 'otherCredits', label: 'Other tax credits, PAYG instalments or ATO account adjustments?', normal: 'no', help: 'The illustration uses salary/wage withholding only. It cannot predict offsets against existing debts or other account transactions.', gap: 'Other payments, credits and ATO account adjustments need separate reconciliation.' },
  { key: 'otherClaims', label: 'Other deductions, offsets, adjustments or earlier-year losses?', normal: 'no', help: 'Excludes the automatically calculated low income tax offset. Include investment losses, exempt foreign employment income, family trust amounts and FHSS releases here if relevant.', gap: 'Other deductions, offsets, adjustments or losses remain unprepared.' },
] as const
...
export function preparationSummary(prep: Preparation, records: Evidence[], links: EvidenceLink[], separate: string[], importIssues: string[], now = new Date()) {
  ...
  gaps.push({ section: 'Review', message: 'A reliable refund/payable estimate remains unavailable pending qualified tax-rule and rounding review. A bounded fictional calculation may be available below; this is not a complete return or a myTax entry guide.' })
