...
export default function DocumentWorkspace() {
  ...
  return <section className="document-workspace" aria-labelledby="documents-title">
    ...
    {!started ? <div className="workspace-setup"><div className="evidence-form"><label>Financial year<select value={year} onChange={event => setYear(event.target.value)}><option value="">Select a year</option><option value="2025-26">2025–26 · 1 Jul 2025–30 Jun 2026</option><option value="other">Another financial year</option></select></label><label>Tax situation<select value={context} onChange={event => setContext(event.target.value)}><option value="">Select a situation</option><option value="employee">Employee · phone-service example</option><option value="complex">Business, rental, investments, foreign income or other circumstances</option></select></label></div>
