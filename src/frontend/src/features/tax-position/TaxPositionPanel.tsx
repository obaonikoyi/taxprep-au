...
export default function TaxPositionPanel({ value }: { value: TaxPosition }) {
  return <section className="tax-position preparation-step" aria-labelledby="tax-position-title">
    ...
    {value.balanceCents !== null ? <>
      <div className="tax-balance"><span>{balanceLabel(value)}</span><strong>{money(Math.abs(value.balanceCents))}</strong><p>Recorded withholding minus the illustrated total of tax and Medicare. This is a scenario result, not an approved tax return or a predicted ATO payment.</p></div>
