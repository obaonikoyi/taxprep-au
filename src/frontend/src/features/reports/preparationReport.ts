...
${rows.map(({ item, expense }) => `<article class="expense"><div class="expense-heading"><h3>${categoryLabels[item.category]}</h3><span class="status">${statusLabels[item.status]}</span></div>
...
${item.actions.length ? `<ul class="item-actions">${item.actions.map(action => `<li>${text(action)}</li>`).join('')}</ul>` : ''}</article>`).join('')}
</section>
<footer><strong>About this copy</strong><p>This is a snapshot of ${rows.length} recorded expense(s) in Sarah's fictional demo. Skipped categories and the separate CSV preview are not included. It is not a complete tax return.</p><p>Later edits or restarting the app do not update or remove this downloaded file. TaxPrep AU is independent of the Australian Taxation Office.</p><p class="version">Report format v1 | ${text(year)} | ${text(displayedTime)}</p></footer>
</main></body></html>`
