...
it('keeps the preparation hub session-only and exposes the bank/expense review flow', () => {
  ...
  expect(screen.getByText('Xoba Paycheck still has no automatic browser or cloud save for this workspace. Refreshing or leaving the workflow clears the in-app preparation state.')).toBeDefined()
...
it('reviews and explicitly applies same-year handoffs without auto-completing rules or double counting amounts', async () => {
  ...
  expect((await screen.findByRole('alert')).textContent).toContain('not the selected 2026–27 financial year')
})
