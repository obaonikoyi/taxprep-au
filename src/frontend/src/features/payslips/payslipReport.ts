 ...
 * been verified, which is exactly what TaxPrep is not in a position to say.
 *
...
function rateSection(scoped: Payslip[], records: RateRecord[]): string {
  ...
  return `<h2>Pay rate checks</h2><p>These cover the payslips inside this report's filters, and compare each checked one with the rate the user recorded and with the payslip's own hours and rate. The app's own screen checks every payslip in the session, so it may list more than this report does. TaxPrep has no knowledge of awards, classifications, penalty rates or rostering, so nothing here establishes what the user is entitled to be paid. Each item is a question to put to the employer's payroll contact.</p>
...
export function downloadPayReport(html: string) {
  ...
  a.href = url; a.download = 'taxprep-pay-history.html'; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000)
}
