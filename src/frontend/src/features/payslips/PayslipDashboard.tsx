...
export default function PayslipDashboard() {
  ...
  return <section className="statement-dashboard payslip-dashboard" aria-label="Payslip dashboard">
    ...
    {slips.length > 0 && <>
      <div className="pay-filter-row"><div><strong>{fictional ? 'Fictional example history' : 'Your pay history'}</strong><p>{confirmed.length} confirmed in this view · {pending.length} awaiting review across this session</p></div><div className="pay-filters"><label>Financial year<select aria-label="Financial year" value={year} onChange={e => setYear(e.target.value)}><option value="all">All years</option>{years.map(y => <option key={y}>{y}</option>)}</select></label><label>Employer<select aria-label="Employer" value={employer} onChange={e => setEmployer(e.target.value)}><option value="all">All employers</option>{employers.map(([key, name]) => <option key={key} value={key}>{name}</option>)}</select></label><label>Chart grouping<select aria-label="Chart grouping" value={grouping} onChange={e => setGrouping(e.target.value as 'month' | 'payday')}><option value="month">Monthly</option><option value="payday">Per payday</option></select></label></div></div>
